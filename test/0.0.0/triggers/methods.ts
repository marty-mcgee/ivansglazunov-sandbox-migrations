...
export const prepareLinks = async (
  data: IData,
  count: number,
  getRandomNodeIndex = (nodes, i) => _.random(0, nodes.length - 1),
  getRandomSafetyTarget = (safetyTargets, i) => _.random(0, safetyTargets.length - 1),
) => {
  const objects = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    const safetyTargets = await selectSafetyLinksFrom(n);
    if (!safetyTargets.length) throw new Error('!safetyTargets.length');
    const r2 = getRandomSafetyTarget(safetyTargets, i);
    randomSafetyTargets.push(r2, i);
    objects.push({
      source_id: n.id,
      target_id: safetyTargets[r2].id,
      type_id: 1,
    });
  }
...
export const selectSafetyLinksFrom = async (node) => {
...
export const check = (data: IData, errors: IError[] = []) => {
  for (let i0 = 0; i0 < data.nodes.length; i0++) {
    ...
    if (node.links_by_target.length) {
      // перейти к родителям, посмотреть на его списки индексов, продлить их нод и сравнить
      let rI = 0;
      let nI = 0;
